...
import { demoPermissionDetails, isDemoBypassEnabled } from '@/lib/demoMode';
import { isAnystoreBackendConfigured } from '@/lib/anystoreBackend';
...
export default function DataroomLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if ((demoMode || isAnystoreBackendConfigured) && dataroomId) {
      setPermissions({
        ...demoPermissionDetails,
        dataroomId,
        canAccessAuditLogsPanel: false,
        canAccessDiligenceDashboard: false,
        canAccessIssuesPanel: false,
        canAccessQuestionairePanel: false,
  ...
  }, [dataroomId, currentDataroomId, demoMode, isAnystoreBackendConfigured]);
  ...
  if (!hasPermission) {
    ...
    return (
      <div className="grid h-screen place-items-center">
        <div className="flex flex-col items-center gap-4">
          <h2 className="text-xl font-semibold">This library does not exist or you do not have access</h2>
