...
import { demoSearchableFiles, demoUser, demoUserAttributes, isDemoBypassEnabled } from '@/lib/demoMode';
...
export default function Home() {
  ...
  const tabs: Tab[] = [
    ...
    // Keep the old workstream panels available in code while AnyStore starts as a simple library.
    // { icon: ChartPie, label: 'Diligence' },
    // { icon: MessagesSquare, label: 'Issues' },
  ...
  useEffect(() => {
    if (contextDataroomId) { // Use dataroomId from context
      if (demoMode) {
        setSearchableFilesInStore(demoSearchableFiles);
  ...
  const fetchSearchableFiles = async (idToFetch: string) => { // Accept ID as parameter
    ...
    try {
      if (isAnystoreBackendConfigured) {
        const documents = await anystoreBackend.getDocuments(idToFetch);
        const formattedFiles: FileItem[] = documents.map((document) => {
          ...
          return {
            ...
            size: document.size_bytes ? `${Math.round(document.size_bytes / 1024)} KB` : '',
