...
const DataRoomCard: React.FC<DataRoomCardProps> = ({ 
  ...
  const handleRetry = async () => {
    ...
    setIsRetrying(true);
    try {
      // First delete the failed library
      ...
      onDelete?.(); // Remove the failed library from the list immediately after deletion
      ...
      onRetry?.(newDataroom); // Add the new library to the list
    } catch (error) {
      console.error('Error retrying library:', error);
    } finally {
      setIsRetrying(false);
    }
  };
