  ...
  const uploadFile = async (file: File, parentFolderId: string, bucketUuid: string) => {
    try {
      if (isAnystoreBackendConfigured) {
        const response = await anystoreBackend.uploadDocument(file, bucketUuid, parentFolderId);
        const documentId = response.document?.id || response.id || `${file.name}-${file.size}`;
        const fileKey = `${file.name}-${file.size}`;
        fileHashes[fileKey] = documentId;
        return true;
      }
      ...
      {isProcessingZip ? (
        ...
        <>
          ...
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-200">
            Drop files, screenshots, receipts, or ZIP archives here
          </p>
          ...
          <div className="mt-4">
            ...
            >
              ...
              {isAuthenticated ? `Import from Microsoft (${user?.name})` : 'Sign in with Microsoft'}
            </Button>
            {isAuthenticated && (
              <Button
                variant="ghost"
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="relative bg-white p-8 rounded-lg shadow-lg w-4/5 max-w-4xl dark:bg-darkbg">
        ...
        <div className="flex flex-col h-[600px]">
          {/* Title */}
          <h3 className="text-lg font-semibold mb-4 dark:text-gray-200">Drop Stuff In</h3>
